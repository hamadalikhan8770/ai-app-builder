...
serve(async (req) => { if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders }); try { const { admin, user } = await context(req); const { token } = await req.json(); const { data: invite, error } = await admin.from("team_invites").select().eq("token", token).eq("status", "pending").maybeSingle(); if (error) throw error; if (!invite) return jsonResponse({ error: "Invite not found." }, 404); if (invite.email.toLowerCase() !== (user.email ?? "").toLowerCase()) return jsonResponse({ error: "This invite belongs to another email." }, 403); await admin.from("team_invites").update({ status: "declined", declined_at: new Date().toISOString() }).eq("id", invite.id); await log(admin, invite.team_id, user.id, "invite_declined", "invite", invite.id); await analytics(admin, user.id, "invite_declined", "project", "none", invite.id); return jsonResponse({ success: true }); } catch (e) { if (e instanceof Response) return e; return jsonResponse({ error: e instanceof Error ? e.message : "Unknown error." }, 500); } });
