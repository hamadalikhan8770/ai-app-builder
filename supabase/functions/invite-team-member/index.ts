...
serve(async (req) => { if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders }); try { const { admin, user } = await context(req); const { teamId, email, role: inviteRole } = await req.json(); if (!teamId || !email || !["admin","editor","viewer"].includes(inviteRole)) return jsonResponse({ error: "teamId, email, and valid role are required." }, 400); if (!canManage(await role(admin, teamId, user.id))) return jsonResponse({ error: "Owner or admin access required." }, 403); const token = randomToken(); const { data: invite, error } = await admin.from("team_invites").insert({ team_id: teamId, email: email.toLowerCase().trim(), role: inviteRole, token, invited_by: user.id }).select().single(); if (error) throw error; await log(admin, teamId, user.id, "member_invited", "invite", invite.id, { email, role: inviteRole }); await analytics(admin, user.id, "member_invited", "project", "none", invite.id, { teamId }); return jsonResponse({ success: true, invite }); } catch (e) { if (e instanceof Response) return e; return jsonResponse({ error: e instanceof Error ? e.message : "Unknown error." }, 500); } });
