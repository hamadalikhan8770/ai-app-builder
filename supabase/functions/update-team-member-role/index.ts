...
serve(async (req) => { if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders }); try { const { admin, user } = await context(req); const { teamId, memberUserId, newRole } = await req.json(); if (!["admin","editor","viewer"].includes(newRole)) return jsonResponse({ error: "Invalid new role." }, 400); if ((await role(admin, teamId, user.id)) !== "owner") return jsonResponse({ error: "Owner access required." }, 403); const targetRole = await role(admin, teamId, memberUserId); if (targetRole === "owner") return jsonResponse({ error: "Transfer ownership before changing owner role." }, 400); const { error } = await admin.from("team_members").update({ role: newRole }).eq("team_id", teamId).eq("user_id", memberUserId); if (error) throw error; await log(admin, teamId, user.id, "member_role_changed", "user", memberUserId, { newRole }); await analytics(admin, user.id, "member_role_changed", "admin", "user", memberUserId); return jsonResponse({ success: true }); } catch (e) { if (e instanceof Response) return e; return jsonResponse({ error: e instanceof Error ? e.message : "Unknown error." }, 500); } });
