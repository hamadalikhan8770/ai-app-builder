...
serve(async (req) => { if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders }); try { const { admin, user } = await context(req); const { teamId, memberUserId } = await req.json(); const callerRole = await role(admin, teamId, user.id); if (!canManage(callerRole) && user.id !== memberUserId) return jsonResponse({ error: "Owner/admin access required." }, 403); const targetRole = await role(admin, teamId, memberUserId); if (targetRole === "owner") return jsonResponse({ error: "Transfer ownership before removing owner." }, 400); if (callerRole === "admin" && targetRole === "admin") return jsonResponse({ error: "Admins cannot remove admins." }, 403); const status = user.id === memberUserId ? "left" : "removed"; const { error } = await admin.from("team_members").update({ status }).eq("team_id", teamId).eq("user_id", memberUserId); if (error) throw error; await log(admin, teamId, user.id, "member_removed", "user", memberUserId, { status }); await analytics(admin, user.id, "member_removed", "admin", "user", memberUserId); return jsonResponse({ success: true }); } catch (e) { if (e instanceof Response) return e; return jsonResponse({ error: e instanceof Error ? e.message : "Unknown error." }, 500); } });
