...
serve(async (req) => { if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders }); try { const { admin, user } = await context(req); const { projectId, teamId, permission } = await req.json(); if (!["view","edit","manage"].includes(permission)) return jsonResponse({ error: "Invalid permission." }, 400); const { data: project } = await admin.from("app_projects").select("id, owner_user_id, user_id").eq("id", projectId).single(); const owns = (project.owner_user_id ?? project.user_id) === user.id; if (!owns && !canManage(await role(admin, teamId, user.id))) return jsonResponse({ error: "Project owner or team manager access required." }, 403); const { data: share, error } = await admin.from("project_shares").upsert({ project_id: projectId, team_id: teamId, shared_by: user.id, permission }, { onConflict: "project_id,team_id" }).select().single(); if (error) throw error; await admin.from("app_projects").update({ visibility: "team", team_id: teamId }).eq("id", projectId); await log(admin, teamId, user.id, "project_shared", "project", projectId, { permission }); await analytics(admin, user.id, "project_shared", "project", "project", projectId, { teamId, permission }); return jsonResponse({ success: true, share }); } catch (e) { if (e instanceof Response) return e; return jsonResponse({ error: e instanceof Error ? e.message : "Unknown error." }, 500); } });
